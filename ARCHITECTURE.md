...
This is a single deployable FastAPI service, internally organized into clear modules
(`api/`/`core`/`db`/`schemas`/`ml`/`features`/`services`), talking to one Postgres database — no
service mesh, no message queue, no separate inference service. That's a deliberate choice for what
this project actually is: a **portfolio project** meant to demonstrate breadth and depth of
engineering understanding to an internship reviewer, not a system built to survive production
scale or traffic. A modular monolith keeps the whole system easy to reason about, easy to run
locally with no orchestration, and easy to explain end to end in an interview — splitting it into
services would add real operational complexity (network calls, partial failure, deployment
coordination) without buying anything this project's actual scale needs.

## How this system runs — three separate paths

These are three genuinely independent ways to run the same codebase. They are not layered on top
of each other, and a given run only ever uses one of them.
...
- **Path 1 (local dev)** is the actual historical development loop for this entire project —
  `uvicorn --reload` and `npm run dev` running directly on the host, against a local (or
  free-tier-hosted) Postgres instance. No Docker anywhere in this path.
- **Path 2 (Docker Compose)** exists purely as an end-of-project packaging deliverable and
  containerization exercise — it was **never** used during this project's real development
  history, and has no connection to Path 3 in either direction. One real gotcha baked into this
  path: the frontend container's `NEXT_PUBLIC_API_URL` is inlined into the client JS bundle at
  `next build` time, and every page that calls the backend runs client-side in the visitor's
...
- **Path 3 (live deployment)** is the actual public demo: Vercel builds the frontend directly from
  ...
  the app-runtime connection goes through Supabase's Transaction pooler, while Alembic migrations
  use the Session pooler instead (a transaction-mode pooler can hand different backend connections
  to different statements within one migration, which doesn't play well with DDL).
...
- **Isolation Forest is fully independent of the XGBoost decision.** It runs on every request
  ...
  fields — never merged into `fraud_probability`/`is_fraud`. This is a direct consequence of Phase
  4's measured finding that Isolation Forest added zero unique true-positive catches on this
  dataset; it's kept as a secondary, clearly-labeled signal rather than a second vote.
- **Persistence is synchronous, in the request path, not a background task.** A DB failure here is
  ...
  `PredictionResponse` — a storage hiccup doesn't turn a working prediction into a 500.
...
|---|---|
| `api/` | Route handlers — thin, no business logic. `predict.py`, `predictions.py` (read/list), `health.py`, `simulator.py`, `stripe_webhooks.py`, `stripe_checkout.py`. |
...
| `services/` | Business logic orchestrating db/ml/features — `prediction_service.py` (the core scoring+persistence pipeline described above), `prediction_read_service.py`, `simulator.py` (the background transaction generator), `stripe_adapter.py` (Stripe → `TransactionInput` mapping). |

### Frontend (`frontend/app/`)

| Page | What it does |
|---|---|
| `/` (Overview) | All-time stat tiles + a live-polling recent-activity feed off `GET /predictions`, filterable by fraud/anomaly status. |
...
|---|---|
| `notebooks/` | Exploratory analysis and the phase-by-phase modeling notebooks (EDA, baseline, Random Forest, XGBoost, model comparison, SHAP, Isolation Forest) — each paired with a `*_FINDINGS.md` write-up. |
...
| `models/` | Serialized trained artifacts (`xgboost_model.pkl`, `model_metadata.json`, `isolation_forest.pkl`) — gitignored everywhere except the three files the live deploy actually needs to load, which are committed specifically so Render's git-based deploy has something to load. |
